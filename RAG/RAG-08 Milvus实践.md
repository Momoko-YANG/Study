...
Docker将会自动拉取所需的镜像并启动三个容器：`milvus-standalone`, `milvus-minio`, 和 `milvus-etcd`。

## 3. 验证安装
可以通过以下方式验证Milvus是否成功启动：
...
- 彻底清理（停止并删除容器）：如果想彻底删除所有数据（包括向量，元数据等），可以执行以下命令：
```bash
...
## Schema
在创建Collection之前，必须先定义它的Schema。`Schema`规定了Collection的数据结构，定义了其中所包含的所有字段（Filed）及其属性。

Shcema通常包括以下几类字段：
- 主键字段（Primary Key Field）：每个Collection必须有且仅有一个主键字段，用于唯一标识每一条数据（实体）。它的值必须是唯一的，通常是整数或字符串类型。
- 向量字段（Vector Field）：用于存储核心的向量数据。一个Collection可以有一个或多个向量字段，以满足多模态等复杂场景的需求。
- 标量字段（Scalar Field）：用于存储除向量之外的元数据，如字符串，数字，布尔值，JSON等。这些字段可以用于过滤查询，实现更精确的检索。

![[Pasted image 20260216161653.png]]
...
目的：
- 提升查询性能：在查询是，可以指定只在一个或几个分区内进行搜索，减少扫描数据量，提升检索速度。
- 数据管理：便于对部分数据进行批量操作，如加载/卸载特定分区到内存，或删除整个分区的数据。
...
索引是一种**为了加速查询而设计的复杂数据结构**。对向量数据创建索引后，Milvus 可以极大地提升向量相似性搜索的速度，代价是会占用额外的存储和内存资源。

Milvus向量索引地内部组件及工作流程：
- 数据结构：这是索引的骨架，定义了向量的组织方式（如HNSW中的图结构）
- 量化：数据压缩技术，通过降低向量精度来减少内存占用和加速计算。
- 结果精炼：在找到初步候选集后，进行更精确的计算以优化最终结果。
...
### 主要向量索引类型
- FLAT（精确查找）
	- 原理：暴力搜索。会计算查询向量与集合中所有向量之间的实际距离，返回最精确的结果。
	- 优点：100%召回率，结果最精准。
	- 缺点：速度慢，内存占用大，不适合海量数据。
	- 使用场景：对精度要求极高，且数据规模较小（百万级以内）的场景。
- IVF系列（倒排文件索引）
	- 原理：它首先通过聚类将所有向量分成多个“桶”(`nlist`)，查询时，先找到最相似的几个“桶”，然后只在这几个桶内进行精确搜索。`IVF_FLAT`、`IVF_SQ8`、`IVF_PQ` 是其不同变体，主要区别在于是否对桶内向量进行了压缩（量化）。
	- 优点：缩小范围，提升检索速度，平衡性能与效果。
	- 缺点：召回率不是100%，因为相关向量可能被分到了未被搜索的桶中。
	- 适用场景：通用场景，尤其适合需要高吞吐量的大规模数据集。
- HNSW（基于图的索引）
	- 原理：构建一个多层的邻近图。查询时从最上层的稀疏图开始，快速定位到目标区域，然后在下层的密集图中进行精确搜索。
	- 优点：检索速度快，召回率高，擅长处理高维数据和低延迟查询。
	- 缺点：内存占用大，构建索引的时间长。
...
### 增强检索
1. 过滤检索：在满足特定条件的向量中，查找最相似的结果。将**向量相似性检索**与**标量字段过滤**结合在一起。
...
3. 多向量混合检索（Hybrid Search）
	- 允许在一个请求中同时检索多个向量字段，并将结果智能地融合在一起。
	- 工作原理：
		- 并行检索：应用针对不同的向量字段分别发起ANN检索请求。
		- 结果融合：Milvus 使用一个重排策略（Reranker）将来自不同检索流的结果合并成一个统一的、更高质量的排序列表。常用的策略有 `RRFRanker`（平衡各方结果）和 `WeightedRanker`（可为特定字段结果加权）。
	- 应用示例：
...
def visualize_results(query_image_path: str, retrieved_images: list, img_height: int = 300)
	"""从检索到的图像列表创建一个全景图用于可视化。"""
	panoramic_width = img_width * row_count
	panoramic_height = img_height * row_count
	panoramic_image = np.full((panoramic_height, panoramic_width, 3), 255, dtype=np.uint8)
	query_display_area = np.full((panoramic_height, img_width, 3), 255, dtype=np.unit8)
	...
	for i, img_path in enumerate(retrieved_images):
		...
		resized_retrived = cv2.resize(retrieved_cv, (img_width - 4, img_height -4))
...
print("成功为向量字段创建 HNSW 索引。") print("索引详情:") print(milvus_client.describe_index(collection_name=COLLECTION_NAME, index_name="vector")) milvus_client.load_collection(collection_name=COLLECTION_NAME) print("已加载 Collection 到内存中。")
...
query_text = "一条龙"
query_vector = encoder.encoder_query(image_path=query_image_path, text=query_text)
...
search_results = milvus_client.search(
	...
	limit = 5,
	search_params={"metric_type":"CONSINE", "params":{"ef":128}}
)[0]
...
`distance` 字段代表了**余弦相似度**，值越接近 1 表示越相似。
